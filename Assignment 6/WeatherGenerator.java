...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for(int r1 = 0; r1 < drywet.length; r1++){
            if((drywet[r1][0] == longitude) && (drywet[r1][1] == latitude)){
                for(int month = 0; month < 12; month++){
                    drywetProbability[month] = drywet[r1][month+2];
                }
            }
        }

        for(int r2 = 0; r2 < drywet.length; r2++){
            if((wetwet[r2][0] == longitude) && (wetwet[r2][1] == latitude)){
                for(int month = 0; month < 12; month++){
                    wetwetProbability[month] = wetwet[r2][month+2];
                }
            }
        }
    }
     ...
     *      Here, forecast should be a size 31 array of 1s and 2s, although the probability is determined, the
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        int[] forecastArr = new int[numberOfDays];
        forecastArr[0] = (StdRandom.uniform() < 0.5) ? 1 : 2;
        for(int day = 1; day < forecastArr.length; day++){
            if(forecastArr[day-1] == 2){
                forecastArr[day] = (StdRandom.uniform() < drywetProbability) ? 1 : 2;
            } else{
                forecastArr[day] = (StdRandom.uniform() < wetwetProbability) ? 1 : 2;
            }
        }
        return forecastArr;
    }
     ...
     *      System.out.println(numberOfWetDryDays(arr, WET)); //prints out 1
     */ 
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int day = 1; day < forecast.length; day++){
            if(forecast[day] == 1){
                if(forecast[day] != forecast[day-1]){
                    if(countDRY > maxDRY){
                        maxDRY = countDRY;
                    }
                    countDRY = 1;
